# This script produces the circos plot, which shows collaboration between units
# modified by LH from:
...
# Pull data from the corresponding data preparation folder
from fac_map_circos import fac_map, df

# Use this to see which units you have (can also see in the test.xlsx printed from data prep file)
...
labels = [
    ...
    #'Compute and Storage', #Always omit C&S from circos plot
    "Clinical Genomics Lund",
    "Clinical Genomics Stockholm",
    "Clinical Genomics Linköping",
    # "Clinical Genomics Umeå", #No collabs with units included (only with C&S)
...
matrix = G.to_numpy()
# if you get an error saying 'not in matrix' or not in G, it's because the
...
def make_ribbon_arc(theta0, theta1):
    ...
    if test_2PI(theta0) and test_2PI(theta1):
        ...
        )  # if this error is raised, it's usually fixed by rearranging the order of the names in the 'labels' list at the start
...
fig.update_layout(autosize=False, width=1350, height=850)
...
if not os.path.isdir("Plots"):
    os.mkdir("Plots")
fig.write_image("Plots/circos_plot.png", scale=3)
