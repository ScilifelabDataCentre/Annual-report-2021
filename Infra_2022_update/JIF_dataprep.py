# data preparation for the updates 2022 plot

import pandas as pd

# information from publication database (labels combined, so nothing double counted)

Pubs_JIF_raw = pd.read_excel(
    "data/infra_1321_comblab.xlsx",
...
# Filter for the appropriate time frame

Pubs_JIF_raw = Pubs_JIF_raw[
    (Pubs_JIF_raw["Year"] > 2012) & (Pubs_JIF_raw["Year"] < 2022)
]
...
## below prints out a file that can be checked to determine whether (uncomment it)
...
# had to manually add some JIF scores (biochim biophys acta has multiple!)

JIF_merge_fullnames = pd.read_excel(
    "data/Check_me_manual_improve_inf.xlsx",
    sheet_name="Sheet1",
    header=0,
    engine="openpyxl",
)
...
JIF_sub_group_inf.to_excel("data/infra_JIF_groups.xlsx")
